...
<?php 
    ...
    if (isset($_POST['submit'])) {
        ...
        $sql = $db->prepare("INSERT INTO `user_data`(`user_Fname`, `user_Lname`, `user_phone` , `user_num`, 
                             ...
                             VALUES ('$Fname','$Lname','$phone','$address','$subdis','$dis','$pv','$zipcode','$group_id')");
        $sql->execute();

        $us_id = $db->query("SELECT `user_id` as id FROM `user_data` WHERE `user_phone` = '$phone' and  `user_Fname` = '$Fname' and `user_Lname` = '$Lname'");
        $us_id->execute();
